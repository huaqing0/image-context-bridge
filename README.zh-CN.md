...
外部 agent 如果会先把原图发给云端模型，那是该 agent 自己的行为，不是 Image Context Bridge 的行为。需要避免这种情况时，可以直接运行 `image2context`，或让 agent 先走 fallback 工作流。
...
`CLI` 指的是终端命令。Claude Code、Codex 或其他 agent 已自动触发 `image-context` Skill 时，无需手动运行 CLI。
...
使用 `--target claude`、`--target codex` 或其他 Skill 目标安装后，支持 Skill 的 agent 可以加载 `image-context`。
